                     ...
                     local_row_idx += 1))
                {
                  ...
                  RD_RegsScope(.module = row_info->module->handle) UI_Parent(row_box)
                  {
                    ...
                    if(rd_setting_b32_from_name(str8_lit("cache_line_markers")) &&
                       row->eval.space.kind == CTRL_EvalSpaceKind_Entity && row_info->view_ui_rule == &rd_nil_view_ui_rule)
                    {
...
rd_push_window_title(Arena *arena)
{
  ...
  String8 result = push_str8f(arena, "%S - %s", project_name, BUILD_TITLE_STRING_LITERAL);
  return result;
}
