...
rd_preferences_save_theme(Arena *arena, RD_CmdKind kind, CFG_Node *parent, String8 name)
{
  if(name.size != 0)
  {
    String8 themes_folder = push_str8f(arena, "%S/%Sraddbg/themes", get_process_info()->user_program_data_path, program_data_folder_prefix_from_os(OperatingSystem_CURRENT));
