...
nv_cache_load_file(NV_Cache *cache, String8 path)
{
  ...
  // read file
  File file_handle = file_open(AccessFlag_Read|AccessFlag_ShareRead, path);
  FileProperties props = properties_from_file(file_handle);
  if(props.size == 0)
  {
  ...
  file_read(file_handle, r1u64(0, file_data.size), file_data.str);
  file_close(file_handle);
...
nv_cache_hot_reload(NV_Cache *cache)
{
  ...
  for(NV_CacheEntry *e = cache->first; e != 0; e = e->next)
  {
    ...
    File fh = file_open(AccessFlag_Read|AccessFlag_ShareRead, e->source_path);
    FileProperties props = properties_from_file(fh);
    file_close(fh);
    ...
    if(props.modified != e->source_timestamp && props.size > 0)
    {
      ...
      File fh2 = file_open(AccessFlag_Read|AccessFlag_ShareRead, e->source_path);
      String8 data = {0};
      data.size = props.size;
      data.str = push_array(new_arena, U8, data.size);
      file_read(fh2, r1u64(0, data.size), data.str);
      file_close(fh2);
